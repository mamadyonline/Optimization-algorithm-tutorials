# Author: Mamady Nabé
# Email: mamady.nabe@outlook.com
# Blog: mamadyonline.github.io

import numpy as np

class PSO(object):
    """
        Class to define PSO algorithm.
    """
    ...
    def update_velocity(self, x, v, p_best, g_best, c0=0.5, c1=1.5, w=0.75):
        ...
        x = np.array(x)
        v = np.array(v)
        assert x.shape == v.shape, 'The position and the velocity must have same shape'
...
if __name__ == '__main__':
    ...
    init_pos_r = [0,0,0]
    PSO_r = PSO(func=rosenbrock, init_pos=init_pos_r, n_particles=50)
    res_r = PSO_r.optimize()
